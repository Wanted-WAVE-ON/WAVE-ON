...
| U-1 | 사용자가 명시적으로 시작한 경우에만 웹 UI에 로컬 카메라 미리보기를 표시할 수 있다. 미리보기 프레임은 브라우저 메모리에서만 몸짓 분석에 쓰고, 서버·DB·파일·분석 서비스로 전송하거나 저장하지 않으며 중지·페이지 종료 시 해제한다. Agent Orb·상태 텍스트는 미리보기와 함께 인식 상태를 전달한다. |
